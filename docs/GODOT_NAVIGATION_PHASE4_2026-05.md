...
- **ホーム**: **`HeaderCard` のみ**に Theme を割当（**ルート `HomeDashboard` には付けない**）。**`Scroll` 以下**は **`CardNews` のみ** `phase4_readonly_core.tres` + **`Phase4SummaryCard`** を付与済み（`ed106c8`）。**それ以外**（`NavColumns`・主要指標・Tasks 等）は **従来の StyleBox のまま**。**HeaderNavRow** は **ボタン数・接続・遷移先のシーン定義を変更していない**（親 `HeaderCard` に Theme が付くため、**見た目**は Theme 継承で変わりうる）。
...
- **本線 `home_dashboard.tscn` の Scroll 以下**では、**`CardNews` のみ** **`Phase4SummaryCard`** を限定適用済み（`ed106c8`）。**ナビ構造の変更ではない**。**`news` の本文・行数・from_python / mock 経路は不変**（**`_join_lines`・`home_dashboard.gd` 不変**）。**HeaderNavRow は本線で維持**。**左サイドナビは本線未実装**。**右サマリー比較scene は本線未接続の参考案**。**sandbox の `CardNewsBody` 級の 1 行ヘッドライン本格導入**は **`.gd` または `news_headline` 等の DTO** を**別タスク**で検討。

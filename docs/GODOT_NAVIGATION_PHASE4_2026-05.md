...
- **`godot/scenes/home_production_wire_preview.tscn`**: **左サイドナビ（大分類）と上部クラブ帯・中央カード密度**を研究する **sandbox**（script なし・固定文言・**F6 で単体実行**）。**`project.godot` の `run/main_scene` は変更しない**（`godot/README.md`「本番ホームワイヤー sandbox」節）。
- **大分類の試験**: sandbox 上では **ホーム / チーム / リーグ / 経営 / クラブ**。**最新（2026-05）**では左レールが**ナビ風**になり、**現在地「ホーム」**が強調されている（見た目のみ。クリック・遷移なし）。
- **10 画面を左に全部並べない**方針のもと、**大分類＋中央カードで選ぶ**方向を sandbox で検証中。
- **ClubBand** に**仮クラブロゴ枠**（SG/LOGO・実画像なし）を置き、**上部クラブ帯**の方向性も並行して検証中。

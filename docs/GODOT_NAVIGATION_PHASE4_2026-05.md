...
- **ホーム**: **`HeaderCard` のみ**に Theme を割当（**ルート `HomeDashboard` には付けない**）。**`Scroll` 以下**は **`CardNews`（`ed106c8`）と `CardNext`（`8676095`）**に `phase4_readonly_core.tres` + **`Phase4SummaryCard`**、**`CardWarnings`（`762f5bc`）**に **`Phase4WarningCard`** を付与済み。**それ以外**（`NavColumns`・主要指標・Tasks・Summary 等）は **従来の StyleBox のまま**。**`CardTasks` は従来どおり**。**HeaderNavRow** は **ボタン数・接続・遷移先のシーン定義を変更していない**（親 `HeaderCard` に Theme が付くため、**見た目**は Theme 継承で変わりうる）。
...
- **本線 `home_dashboard.tscn` の Scroll 以下**では、**`CardNews`（`ed106c8`）・`CardNext`（`8676095`）**に **`Phase4SummaryCard`** を限定適用済みに加え、**`CardWarnings`（`762f5bc`）**にも **`Phase4WarningCard`** を限定適用済み。**ナビ構造の変更ではない**。**`news` / `next_game` / `warnings` の本文・表示/非表示ロジック・from_python / mock 経路は不変**（**`_join_lines`・`_set_optional_row`・`_warnings_card.visible`・`home_dashboard.gd` 不変**）。**HeaderNavRow は本線で維持**。**左サイドナビは本線未実装**。**右サマリー比較scene は本線未接続の参考案**。**sandbox の `CardNewsBody` 級の 1 行ヘッドライン本格導入**は **`.gd` または `news_headline` 等の DTO** を**別タスク**で検討。**本線移行は今後も小さなコミット単位**で行う。

...
- **ロスター閲覧**: **Theme 第1段**（`f866f5b`・**`roster_view.tscn` のみ**）＋**表 Theme 通常 Table 化**（`407f014`・**`roster_view.gd` のみ**）＋**第2段（最小）RowList 選手行間 HSeparator**（`8a95fcf`・**`_apply_snapshot` players ループのみ**）＋**ロスター本格整備・第1手 平坦行背景**（`9445d0e`・**`_add_player_row` のみ** — §2.13b）。**`Scroll/TableCard`** の Phase4 化 ＋ **`Phase4TableHead` / `Phase4TableCell`**（§2.13 / §2.13a / §2.13b）。
...
- **ロスター閲覧**: **Theme 第1段**（`f866f5b`・**`roster_view.tscn` のみ**）＋**表 Theme 通常 Table 化**（`407f014`・**`roster_view.gd` のみ**）＋**第2段（最小）RowList 選手行間 HSeparator**（`8a95fcf`）＋**ロスター本格整備・第1手 平坦行背景**（`9445d0e`）。**`TableCard` + 9列表** の Phase4 化（**Header は既に Phase4 済み**）。**選手行カード化・9列本格整理は未変更**（§2.13 / §2.13a / §2.13b）。

...
- **日程閲覧**: **Theme 第1段**（`440c3f6`）＋**第2段・前半**（`986c4ab`）＋**第2段・後半（最小）**（`7fecb99`）＋**追加最小 advance_hint**（`a62b3a7`・**なし/あり表示確認済み**）＋**追加最小 empty_message**（`463e74b`）。**導線は HeaderNavRow + CardNavMenu から到達**（§2.8〜2.8e）。**LeftRail からは遷移しない**。
...
- **Theme 適用範囲（第2段・追加最小）**: **`_add_empty_message_block` の runtime `PanelContainer`**＝`Phase4SummaryCard`。**`_add_upcoming_block` / `_add_advance_hint_block` / NextGameCard / SummaryCard / HeaderCard は今回未変更**。**notes / Footer は対象外**。**見出し全体 / ScrollContent 全体整理は別タスク**。
- **HeaderNavRow / CardNavMenu / LeftRail の役割分担は維持**（§2.4）。

...
次の **5 画面**まで、**Python の DTO → JSON → Godot で表示** の形でつながっています。
...
- **ホーム**の上部に **`HeaderNavRow`** があり、横に **4 つのボタン** があります。  
  **ロスター閲覧** / **クラブ史閲覧** / **順位表閲覧** / **日程閲覧** へ飛びます。
...
- ホーム上部に **横長のボタンを足し続ける** 方式は、**5 画面の時点で限界に近い**（解像度 1280 幅などでは特に）。
- **第 6 画面** を同じやり方で足すと、**横幅・文字の見やすさ**がさらに厳しくなる見込みが高い。
...
- **短期の安全策**。例:「ロスター」「クラブ史」「順位表」「日程」のように短くする。
- **第 6 画面前のつなぎ**としては有効。
...
- **ヘッダーの 4 ボタン** と **カード** が並ぶと **二重導線** になるので、どちらを正とするか整理が必要。
...
- **画面を足す速度**は保てる。
- 一方で **ナビの負債**（詰まったヘッダー、重複）が増えやすい。**現時点では非推奨寄り**（方針を固めてからの方が安全）。
...
- **第 6 画面を足す前に**、少なくとも **ホーム上部ナビの整理**（文言短さ・行の分け方など）を検討する。
- **次に手を入れる実装候補**の第一線は、**「HeaderNavRow の文言短縮・仮整理」** か **「ホームにメニュー用カードを足す（小さく始める）」**。
...
- **既存 5 画面を一度に壊さない** ように、**少しずつ** 導線を整理する。
...
3. **その次** は **「ホームのカード型メニュー（小さく）」** か、**「第 6 画面に何を置くかの調査・判断」** のどちらか（チームで優先度を決める）。
4. **左サイドメニュー**は、**いま全面実装しない**。試すなら **1 画面だけ** や、**別ブランチ** など、**本線の 5 画面確認を壊しにくい**やり方にする（タイミングは **未確認**）。
...
| 第二候補 | **ホームのカード型メニュー**の小さなプロトタイプ |
| 第三候補 | **第 6 画面**に何を置くかの **調査・判断** |

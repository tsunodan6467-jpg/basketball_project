...
- **ロスター閲覧**: **Theme 第1段**（`f866f5b`・**`roster_view.tscn` のみ**）＋**表 Theme 通常 Table 化**（`407f014`・**`roster_view.gd` のみ**）＋**第2段（最小）RowList 選手行間 HSeparator**（`8a95fcf`・**`_apply_snapshot` players ループのみ**）＋**ロスター本格整備・第1手 平坦行背景**（`9445d0e`・**`_add_player_row` のみ** — §2.13b）＋**ロスター本格整備・第2手 主要列強調**（`6c1e25f`・**`_add_player_row` のみ** — §2.13c）＋**ロスター本格整備・第3手 状態列視認補助**（`746e861`・**`_add_player_row` のみ** — §2.13d）。**`Scroll/TableCard`** の Phase4 化 ＋ **`Phase4TableHead` / `Phase4TableCell`**（§2.13 / §2.13a / §2.13b / §2.13c / §2.13d）。
...
- **日程閲覧**: **Theme 第1段**（`440c3f6`）＋**第2段・前半**（`986c4ab`）＋**第2段・後半（最小）**（`7fecb99`）＋**追加最小 advance_hint**（`a62b3a7`・**なし/あり表示確認済み**）＋**追加最小 empty_message**（`463e74b`）＋**追加最小「今後の予定」見出し**（`a24cf6f`）＋**追加最小 upcoming 試合間 HSeparator 整理**（`a9fa054`）＋**中規模改善第1手 upcoming 試合カード内情報階層**（`fa36271` — §2.8h）＋**日程本格整備・第2手 advance_hint 情報階層**（`5a98e31` — §2.8i）＋**日程本格整備・第3手 empty_message 本文情報階層**（`065197b` — §2.8j）。**導線は HeaderNavRow + CardNavMenu から到達**（§2.8〜2.8j）。**LeftRail からは遷移しない**。
- **財務サマリー閲覧**: **Theme 第1段**（`4b43da5`）＋**履歴行文字色最小補正**（`6c3dc43`）＋**第2段（最小）HistoryBody 行区切り**（`d57b021`）＋**Body本格・最小 HistoryBody 内側余白**（`307e719`）＋**Body本格整備・中規模第1手 平坦 Panel 行ラップ**（`c762d88`・**`_fill_history_rows` 履歴行ループのみ** — §2.9c）。**`%HistoryBody` 構造全面整理は別タスク**（§2.9a / §2.9b / §2.9c）。
- **オーナーミッション / クラブ評価閲覧**: **Theme 第1段**（`e6acce0`）＋**今季ミッション動的行文字色最小補正**（`2f808e5`）＋**第2段（最小）MissionsBody 行区切り**（`5a3ae2c`）＋**Body本格・最小 MissionsBody 内側余白**（`d4c0372`）＋**Body本格整備・中規模第2手 平坦 Panel 行ラップ**（`1316a16`・**`_fill_mission_rows` ミッション行ループのみ** — §2.10a / §2.10b / §2.10c）。**`%MissionsBody` 構造全面整理は別タスク**。
...
- **契約 / 人事サマリー閲覧**: **Theme 残り第1段**（`5d1afa2`）＋**RiskRows / PlayerRows 動的行文字色最小補正**（`1df4820`）＋**第2段（最小）PlayerRows / RiskRows 行区切り**（`6b26fa3` / `97b26a8`）＋**Body本格・最小 PlayerRows / RiskRows 内側余白**（`f19ed9b` / `420f240` — §2.12a / §2.12b / §2.12c / §2.12d）。**RiskCard / PlayersCard** の Phase4 化。**契約・人事の PlayerRows / RiskRows 最小行区切りは両方完了**。**Body余白横展開レーン 5 Body 完了**（**ナビ構造の完了ではない** — §2.12 / §2.12a〜§2.12d）。
- **ロスター閲覧**: **Theme 第1段**（`f866f5b`・**`roster_view.tscn` のみ**）＋**表 Theme 通常 Table 化**（`407f014`・**`roster_view.gd` のみ**）＋**第2段（最小）RowList 選手行間 HSeparator**（`8a95fcf`）＋**ロスター本格整備・第1手 平坦行背景**（`9445d0e`）＋**ロスター本格整備・第2手 主要列強調**（`6c1e25f`）＋**ロスター本格整備・第3手 状態列視認補助**（`746e861`）。**`TableCard` + 9列表** の Phase4 化（**Header は既に Phase4 済み**）。**選手行カード化・9列本格整理は未変更**（§2.13 / §2.13a / §2.13b / §2.13c / §2.13d）。
...
### 2.8h 日程閲覧・upcoming 試合カード内情報階層整理と導線確認（`fa36271`）

- **到達点**: **`schedule_view.gd` の `_add_upcoming_block` のみ**（**ナビ構造の変更ではない**・**Body余白横展開後の中規模改善第1手**・**試合カード内情報階層整理**）。**`schedule_view.tscn`・Theme `.tres`・`project.godot` 未変更**。**`_fill_scroll_body`・他 `_add_*` 未変更**。
- **ホームからの到達**（**変更なし**）:
  - **`HeaderNavRow`** — 5 導線のうち **日程閲覧**。
  - **`CardNavMenu`** — **リーグ**列の **日程** ボタン（**HeaderNavRow と二重導線**）。
  - **LeftRail からは遷移しない**（表示のみ — §2.4）。**LeftRail は大分類表示であり、日程画面へのショートカットではない**。
- **日程からホームへ**: **`HomeNavButton`**（**`HeaderNavRow` 内**）→ **`_on_home_nav_button_pressed`**。**node 名 / text / tooltip / connection / handler 名は維持**（`fa36271` 後もユーザー確認済み）。
- **ユーザー環境 Godot（ローカル目視）**: **`schedule_from_python.json` 一時退避 → 同梱 mock 読込 → 復元済み**。**DataSourceLabel**＝**同梱モックJSON OK**。**ホーム → 日程遷移 OK**。**upcoming 上段メタ（round / 大会 / H-A）横並び OK**。**対戦行の読みやすさ OK**。**detail 副情報 OK**。**upcoming 件数 / 順序 / 文言維持 OK**。**「今後の予定」見出し・advance_hint / empty_message / notes / Footer 維持 OK**。**Header / Summary / NextGame 維持 OK**。**HomeNavButton でホームへ戻る OK**。**エラーなし・復元後差分なし**。
- **Theme 適用範囲（中規模改善・第1手）**: **`_add_upcoming_block` 内** — line1 **HBox + 3 Label**、line2 **対戦主役**（font_size 16）、line3 **副情報**。**Panel 外枠は `Phase4SummaryCard` 維持**。**ScrollContent 全体・試合リスト本格全体の完了を意味しない**。
- **HeaderNavRow / CardNavMenu / LeftRail の役割分担は維持**（§2.4）:
  - **HeaderNavRow**＝上部主要 5 導線（日程含む）。
  - **CardNavMenu**＝実操作用中央画面メニュー（**リーグ → 日程**が併用入口）。
  - **LeftRail**＝表示のみ（**日程へ直接遷移しない**）。

### 2.8i 日程閲覧・advance_hint ブロック内情報階層整理と導線確認（`5a98e31`）

- **到達点**: **`schedule_view.gd` の `_add_advance_hint_block` のみ**（**ナビ構造の変更ではない**・**日程本格整備の中規模第2手**・**advance_hint ブロック内情報階層整理**）。**`schedule_view.tscn`・Theme `.tres`・`project.godot` 未変更**。**`_fill_scroll_body`・upcoming・他 `_add_*` 未変更**。
- **ホームからの到達**（**変更なし**）:
  - **`HeaderNavRow`** — 5 導線のうち **日程閲覧**。
  - **`CardNavMenu`** — **リーグ**列の **日程** ボタン（**HeaderNavRow と二重導線**）。
  - **LeftRail からは遷移しない**（表示のみ — §2.4）。**LeftRail は大分類表示であり、日程画面へのショートカットではない**。
- **日程からホームへ**: **`HomeNavButton`**（**`HeaderNavRow` 内**）→ **`_on_home_nav_button_pressed`**。**node 名 / text / tooltip / connection / handler 名は維持**（`5a98e31` 後もユーザー確認済み）。
- **ユーザー環境 Godot（ローカル目視）**: **advance_hint 白カード表示 OK**。**見出し「進行ヒント（advance_hint）」維持 OK**。**block 主役化 OK**。**one_line 副情報化 OK**。**block / one_line 文言維持 OK**。**空時非表示維持 OK**。**upcoming 維持 OK**。**empty_message / notes / Footer 維持 OK**。**Header / Summary / NextGame 維持 OK**。**DataSourceLabel 維持 OK**。**HomeNavButton でホームへ戻る OK**。**エラーなし・実行後 git 差分なし**。
- **Theme 適用範囲（本格整備・第2手）**: **`_add_advance_hint_block` 内** — **block 主役**（font_size 15・濃色）、**one_line 副情報**（font_size 11・控えめ色）、inner **separation 6**。**Panel 外枠は `Phase4SummaryCard` 維持**。**日程 ScrollContent 全体・試合リスト本格全体の完了を意味しない**。
- **HeaderNavRow / CardNavMenu / LeftRail の役割分担は維持**（§2.4）。

### 2.8j 日程閲覧・empty_message（お知らせ）本文情報階層整理と導線確認（`065197b`）

- **到達点**: **`schedule_view.gd` の `_add_empty_message_block` のみ**（**ナビ構造の変更ではない**・**日程本格整備の中規模第3手**・**empty_message（お知らせ）本文の情報階層整理**）。**`schedule_view.tscn`・Theme `.tres`・`project.godot` 未変更**。**`_fill_scroll_body`・upcoming・advance_hint・他 `_add_*` 未変更**。
- **ホームからの到達**（**変更なし**）:
  - **`HeaderNavRow`** — 5 導線のうち **日程閲覧**。
  - **`CardNavMenu`** — **リーグ**列の **日程** ボタン（**併用入口**）。
  - **LeftRail からは遷移しない**（表示のみ — §2.4）。
- **日程からホームへ**: **`HomeNavButton`**（**`HeaderNavRow` 内**）→ **`_on_home_nav_button_pressed`**。**node 名 / text / tooltip / connection / handler 名は維持**（`065197b` 後もユーザー確認済み）。
- **ユーザー環境 Godot（ローカル目視）**: **ホーム → 日程遷移 OK**。**empty_message（お知らせ）カード表示 OK**。**見出し「お知らせ」維持 OK**。**本文の読みやすさ OK**。**本文文言維持 OK**。**表示 / 非表示条件維持 OK**。**upcoming 維持 OK**。**advance_hint 維持 OK**。**notes / Footer 維持 OK**。**Header / Summary / NextGame 維持 OK**。**DataSourceLabel 維持 OK**。**HomeNavButton でホームへ戻る OK**。**エラーなし・実行後 git 差分なし**。
- **Theme 適用範囲（本格整備・第3手）**: **`_add_empty_message_block` 内** — 見出し「お知らせ」維持（font 17）、**本文主役化**（font_size 15・`Color(0.08, 0.11, 0.18, 1)`）、inner **separation 6**。**Panel 外枠は `Phase4SummaryCard` 維持**。**日程 ScrollContent 全体・試合リスト本格全体・日程本格整備全体の完了を意味しない**。
- **HeaderNavRow / CardNavMenu / LeftRail の役割分担は維持**（§2.4）。
...
- **可読性補正（`6c3dc43`）**: **`%HistoryBody`** に追加される動的履歴行 `Label` の色のみ **`Color(0.16, 0.2, 0.3)`** へ。**HistoryBody 構造・履歴行レイアウト本格整理は第2段**（**第2段・最小 `d57b021` で行区切り**、**Body本格・最小 `307e719` で内側余白**、**Body本格整備・中規模第1手 `c762d88` で平坦 Panel 行ラップ** — §2.9a / §2.9b / §2.9c）。
...
### 2.9c 財務サマリー閲覧・Body本格整備・中規模第1手（平坦 Panel 行ラップ）と導線確認（`c762d88`）

- **到達点**: **`finance_summary_view.gd` の `_fill_history_rows` 履歴行追加ループのみ**（**ナビ構造の変更ではない**・**Body本格整備・中規模第1手**・**HistoryBody 履歴行の平坦 Panel 行ラップ**）。**`finance_summary_view.tscn`・Theme `.tres`・project.godot 未変更**。**Summary / finance_items / notes 未変更**。
- **ホームからの到達**（**変更なし**）:
  - **`CardNavMenu`** — **経営**列の **財務サマリー** ボタン（**#7・主入口**）。
  - **`HeaderNavRow` には載せない**（§3）。
  - **LeftRail からは遷移しない**（表示のみ — §2.4）。**LeftRail は大分類表示であり、財務サマリー画面へのショートカットではない**。
- **財務サマリーからホームへ**: **`HomeNavButton`**（**`HeaderCard/HeaderInner/HeaderTopRow` 内**）→ **`_on_home_nav_button_pressed`** → **`home_dashboard.tscn`**。**node 名 / text / tooltip / connection / handler 名は維持**（`c762d88` 後もユーザー確認済み）。
- **ユーザー環境 Godot（ローカル目視）**: **ホーム → 財務サマリー遷移 OK**。**HistoryBody 履歴行 Panel 表示 OK**。**行背景が派手すぎない OK**。**履歴文言 / 件数 / 順序 / 空表示維持 OK**。**HSeparator 維持 OK**。**Summary / finance_items / notes 維持 OK**。**HeaderCard / 5静的カード維持 OK**。**DataSourceLabel 維持 OK**。**HomeNavButton でホームへ戻る OK**。**エラーなし・実行後 git 差分なし**。
- **Theme 適用範囲（Body本格整備・中規模第1手）**: **`_fill_history_rows` 内** — **PanelContainer → MarginContainer → Label**、**StyleBoxFlat** 平坦行背景（**`9445d0e` 同系**）。**履歴文言・件数・順序・JSON key・HSeparator は不変**。**5 Body 横展開・Body本格整備全体の完了を意味しない**。
- **Composer 2.5 指定**: **ナビ・導線への不要変更なし**（実装報告ベース）。
- **HeaderNavRow / CardNavMenu / LeftRail の役割分担は維持**（§2.4）。
...
- **可読性補正（`2f808e5`）**: **`%MissionsBody`** に追加される動的ミッション行 `Label` の色のみ **`Color(0.16, 0.2, 0.3, 1)`** へ。**MissionsBody 構造・ミッション行レイアウト本格整理は第2段**（**第2段・最小 `5a3ae2c` で行区切り**、**Body本格・最小 `d4c0372` で内側余白**、**Body本格整備・中規模第2手 `1316a16` で平坦 Panel 行ラップ** — §2.10a / §2.10b / §2.10c）。
...
### 2.10c オーナーミッション / クラブ評価閲覧・Body本格整備・中規模第2手（平坦 Panel 行ラップ）と導線確認（`1316a16`）

- **到達点**: **`owner_mission_view.gd` の `_fill_mission_rows` ミッション行追加ループのみ**（**ナビ構造の変更ではない**・**Body本格整備・中規模第2手**・**MissionsBody ミッション行の平坦 Panel 行ラップ**）。**`owner_mission_view.tscn`・Theme `.tres`・project.godot 未変更**。**Summary / notes 未変更**。
- **ホームからの到達**（**変更なし**）:
  - **`CardNavMenu`** — **経営**列の **オーナーミッション** ボタン（**#8・主入口**）。
  - **`HeaderNavRow` には載せない**（§3）。
  - **LeftRail からは遷移しない**（表示のみ — §2.4）。**LeftRail は大分類表示であり、オーナーミッション画面へのショートカットではない**。
- **オーナーミッションからホームへ**: **`HomeNavButton`**（**`HeaderCard/HeaderInner/HeaderTopRow` 内**）→ **`_on_home_nav_button_pressed`** → **`home_dashboard.tscn`**。**node 名 / text / tooltip / connection / handler 名は維持**（`1316a16` 後もユーザー確認済み）。
- **ユーザー環境 Godot（ローカル目視）**: **ホーム → オーナーミッション遷移 OK**。**MissionsBody 行 Panel 表示 OK**。**行背景が派手すぎない OK**。**ミッション文言 / 件数 / 順序 / 空表示維持 OK**。**HSeparator 維持 OK**。**Summary / notes 維持 OK**。**HeaderCard / 4静的カード維持 OK**。**DataSourceLabel 維持 OK**。**HomeNavButton でホームへ戻る OK**。**エラーなし・実行後 git 差分なし**。
- **Theme 適用範囲（Body本格整備・中規模第2手）**: **`_fill_mission_rows` 内** — **PanelContainer → MarginContainer → Label**、**StyleBoxFlat** 平坦行背景（**財務 `c762d88` 同系**）。**ミッション文言・件数・順序・JSON key・HSeparator は不変**。**5 Body 横展開・Body本格整備全体の完了を意味しない**。
- **HeaderNavRow / CardNavMenu / LeftRail の役割分担は維持**（§2.4）。
...
### 2.12d 契約 / 人事サマリー閲覧・Theme 第2段（Body本格・最小）RiskRows 内側余白と導線確認（`420f240`）

- **到達点**: **`contract_personnel_summary_view.gd` の `_fill_risk_rows` 人事リスク行追加ループ部分のみ**（**ナビ構造の変更ではない**・**Body本格整備の余白横展開第4号（レーン締め）**）。**`contract_personnel_summary_view.tscn`・Theme `.tres`・`project.godot` 未変更**。**`_fill_player_rows` / PlayerRows は `f19ed9b` のまま未変更**。**HeaderCard / 静的5カード・DataSourceLabel 未変更**。
- **ホームからの到達**（**変更なし**）:
  - **`CardNavMenu`** — **経営**列の **契約・人事サマリー** ボタン（**#10・主入口**）。
  - **`HeaderNavRow` には載せない**（§3）。
  - **LeftRail からは遷移しない**（表示のみ — §2.4）。**LeftRail は大分類表示であり、契約・人事サマリー画面へのショートカットではない**。
- **契約・人事サマリーからホームへ**: **`HomeNavButton`**（**`HeaderCard/HeaderInner/HeaderTopRow` 内**）→ **`_on_home_nav_button_pressed`** → **`home_dashboard.tscn`**。**node 名 / text / tooltip / connection / handler 名は維持**（`420f240` 後もユーザー確認済み）。
...
- **Theme 適用範囲（Body本格・最小）**: **`%RiskRows` 内動的人事リスク行**の **`MarginContainer` 内側余白**（財務 `307e719` / OM `d4c0372` / 戦術 `2c637f2` / 契約 PlayerRows `f19ed9b` と同値。**Panel 化・行カード化・本格レイアウトは別タスク**。**`%PlayerRows` は `f19ed9b` のまま**）。
- **Body余白横展開レーン**: **5 Body 完了**（**財務 HistoryBody / OM MissionsBody / 戦術 PlayerRolesBody / 契約 PlayerRows / 契約 RiskRows**）。**これはナビ構造の完了を意味しない** — **HeaderNavRow / CardNavMenu / LeftRail の役割分担は維持**（§2.4）:
  - **HeaderNavRow**＝上部主要 5 導線（契約・人事サマリーは載せない）。
  - **CardNavMenu**＝実操作用中央画面メニュー（**経営 → 契約・人事サマリー**が主入口）。
  - **LeftRail**＝表示のみの大分類ナビ（**契約・人事サマリーへ直接遷移しない**）。
- **進行方針（`420f240` 確認記録時点）**: **細かい行余白・区切り微調整は一区切り**。**次は中規模以上**（ロスター本格・日程本格・Body中規模整理・ゲーム体験寄り機能 / 画面）。
...
- **表 Theme 切替（`407f014`）**: 動的表 `Label` を **`Phase4TableHead` / `Phase4TableCell`** へ。**選手行カード化・9列本格整理は未変更**（**平坦行背景は `9445d0e` — §2.13b**）。
...
### 2.13b ロスター閲覧・ロスター本格整備・第1手（平坦行背景）と導線確認（`9445d0e`）

- **到達点**: **`roster_view.gd` の `_add_player_row` のみ**（**ナビ構造の変更ではない**・**ロスター本格整備の中規模第1手**・**RowList 選手行の平坦 PanelContainer 行背景ラップ**）。**`roster_view.tscn`・Theme `.tres`・project.godot 未変更**。**`_apply_snapshot` 未変更**。
- **ホームからの到達**（**変更なし**）:
  - **`HeaderNavRow`** — **5 導線の先頭**（**ロスター**）。
  - **`CardNavMenu`** — **チーム**列の **ロスター** ボタン（**#1・併用入口**）。
  - **LeftRail からは遷移しない**（表示のみ — §2.4）。**LeftRail は大分類表示であり、ロスター画面へのショートカットではない**。
- **ロスターからホームへ**: **`HomeNavButton`**（**`HeaderCard/HeaderInner/HeaderNavRow` 内**）→ **`_on_home_nav_button_pressed`** → **`home_dashboard.tscn`**。**node 名 / text / tooltip / connection / handler 名は維持**（`9445d0e` 後もユーザー確認済み）。
- **ユーザー環境 Godot（ローカル目視）**: **ホーム → ロスター遷移 OK**。**RowList 各選手行の平坦な行背景 OK**。**9列内容 / 順序 / 幅維持 OK**。**tooltip 維持 OK**。**RowList 行間 HSeparator 維持 OK**（**`8a95fcf` の状態を維持**）。**最終行後の不要 HSeparator なし OK**。**Header行 / Header下Separator 維持 OK**。**HeaderCard / TableCard 維持 OK**。**DataSourceLabel 維持 OK**。**HomeNavButton でホームへ戻る OK**。**エラーなし・実行後 git 差分なし**。
- **Theme 適用範囲（本格整備・第1手）**: **`_add_player_row` 内** — 既存 **9列 HBox** を **`PanelContainer` + `StyleBoxFlat`** で平坦な行背景ラップ（**`Phase4SummaryCard` は使わない**）。**9列・列幅・tooltip・文言・JSON key は不変**。**Panel 化・選手行カード化・9列本格整理は別タスク**。**ロスター本格整備全体の完了を意味しない**。
- **HeaderNavRow / CardNavMenu / LeftRail の役割分担は維持**（§2.4）。

### 2.13c ロスター閲覧・ロスター本格整備・第2手（主要列強調）と導線確認（`6c1e25f`）

- **到達点**: **`roster_view.gd` の `_add_player_row` のみ**（**ナビ構造の変更ではない**・**ロスター本格整備の中規模第2手**・**RowList 選手行の氏名列・OVR列の主要列強調**）。**`roster_view.tscn`・Theme `.tres`・project.godot 未変更**。**`_apply_snapshot` 未変更**。
- **ホームからの到達**（**変更なし**）:
  - **`HeaderNavRow`** — **5 導線の先頭**（**ロスター**）。
  - **`CardNavMenu`** — **チーム**列の **ロスター** ボタン（**#1・併用入口**）。
  - **LeftRail からは遷移しない**（表示のみ — §2.4）。**LeftRail は大分類表示であり、ロスター画面へのショートカットではない**。
- **ロスターからホームへ**: **`HomeNavButton`**（**`HeaderCard/HeaderInner/HeaderNavRow` 内**）→ **`_on_home_nav_button_pressed`** → **`home_dashboard.tscn`**。**node 名 / text / tooltip / connection / handler 名は維持**（`6c1e25f` 後もユーザー確認済み）。
- **ユーザー環境 Godot（ローカル目視）**: **ホーム → ロスター遷移 OK**。**氏名列の読みやすさ OK**。**OVR列の読みやすさ OK**。**9列内容 / 順序 / 幅維持 OK**。**tooltip 維持 OK**。**RowList 行背景維持 OK**（**`9445d0e` の状態を維持**）。**RowList 行間 HSeparator 維持 OK**（**`8a95fcf` の状態を維持**）。**最終行後の不要 HSeparator なし OK**。**Header行 / Header下Separator 維持 OK**。**HeaderCard / TableCard 維持 OK**。**DataSourceLabel 維持 OK**。**HomeNavButton でホームへ戻る OK**。**エラーなし・実行後 git 差分なし**。
- **Theme 適用範囲（本格整備・第2手）**: **`_add_player_row` 内** — **氏名列（`i == 1`）** と **OVR列（`i == 4`）** の **`font_size` 13** / **`font_color` `Color(0.08, 0.11, 0.18, 1)`** のみ上書き。**9列・列幅・tooltip・文言・JSON key・行背景は不変**。**選手行カード化・9列本格整理は別タスク**。**ロスター本格整備全体の完了を意味しない**。
- **HeaderNavRow / CardNavMenu / LeftRail の役割分担は維持**（§2.4）。

### 2.13d ロスター閲覧・ロスター本格整備・第3手（状態列視認補助）と導線確認（`746e861`）

- **到達点**: **`roster_view.gd` の `_add_player_row` のみ**（**ナビ構造の変更ではない**・**ロスター本格整備の中規模第3手**・**RowList 選手行の状態列（`i == 8`）視認補助**）。**`roster_view.tscn`・Theme `.tres`・project.godot 未変更**。**`_apply_snapshot` 未変更**。
- **ホームからの到達**（**変更なし**）:
  - **`HeaderNavRow`** — **5 導線の先頭**（**ロスター**）。
  - **`CardNavMenu`** — **チーム**列の **ロスター** ボタン（**#1・併用入口**）。
  - **LeftRail からは遷移しない**（表示のみ — §2.4）。**LeftRail は大分類表示であり、ロスター画面へのショートカットではない**。
- **ロスターからホームへ**: **`HomeNavButton`**（**`HeaderCard/HeaderInner/HeaderNavRow` 内**）→ **`_on_home_nav_button_pressed`** → **`home_dashboard.tscn`**。**node 名 / text / tooltip / connection / handler 名は維持**（`746e861` 後もユーザー確認済み）。
- **ユーザー環境 Godot（ローカル目視）**: **ホーム → ロスター遷移 OK**。**状態列の拾いやすさ OK**。**氏名列 / OVR列維持 OK**。**9列内容 / 順序 / 幅維持 OK**。**tooltip 維持 OK**。**mouse_filter 維持 OK**。**RowList 行背景維持 OK**（**`9445d0e` の状態を維持**）。**RowList 行間 HSeparator 維持 OK**（**`8a95fcf` の状態を維持**）。**最終行後の不要 HSeparator なし OK**。**Header行 / Header下Separator 維持 OK**。**HeaderCard / TableCard 維持 OK**。**DataSourceLabel 維持 OK**。**HomeNavButton でホームへ戻る OK**。**エラーなし・実行後 git 差分なし**。
- **Theme 適用範囲（本格整備・第3手）**: **`_add_player_row` 内** — **状態列（`i == 8`）** の **`font_size` 13** / **`font_color` `Color(0.08, 0.11, 0.18, 1)`** のみ上書き（**氏名列 / OVR列の既存強調は維持**）。**9列・列幅・tooltip・文言・JSON key・行背景は不変**。**選手行カード化・9列本格整理は別タスク**。**ロスター本格整備全体の完了を意味しない**。
- **HeaderNavRow / CardNavMenu / LeftRail の役割分担は維持**（§2.4）。

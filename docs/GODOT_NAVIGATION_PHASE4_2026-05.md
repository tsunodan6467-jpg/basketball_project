...
- **ホーム**: **`HeaderCard` のみ**に Theme を割当（**ルート `HomeDashboard` には付けない**）。**`Scroll` 以下**は **`CardNews`（`ed106c8`）・`CardNext`（`8676095`）・`CardTasks`（`d18bf1f`）**に `phase4_readonly_core.tres` + **`Phase4SummaryCard`**、**`CardWarnings`（`762f5bc`）**に **`Phase4WarningCard`** を付与済み（**`CardWarnings`＝警告、`CardTasks`＝行動・ToDo** の分界）。**それ以外**（`NavColumns`・主要指標・Summary 等）は **従来の StyleBox のまま**。**HeaderNavRow** は **ボタン数・接続・遷移先のシーン定義を変更していない**（親 `HeaderCard` に Theme が付くため、**見た目**は Theme 継承で変わりうる）。

...
次の **6 画面**まで、**Python の DTO → JSON → Godot で表示** の形でつながっています。
...
- **ホーム**の上部に **`HeaderNavRow`** があり、横に **5 つのボタン** があります。  
  **ロスター閲覧** / **クラブ史閲覧** / **順位表閲覧** / **日程閲覧** / **施設サマリー閲覧** へ飛びます。
- **ホーム内のカード型メニュー（読み取り）**: **チーム**（ロスター）／**リーグ**（順位表・日程）／**クラブ**（クラブ史・**施設**）の入り口。**施設サマリー**は **クラブ寄りの読み取り画面**として現時点では **クラブ列**に置いている（将来 **経営** カテゴリへ寄せる余地は **未確認**）。
...
- ホーム上部に **横長のボタンを足し続ける** 方式は、**6 画面の時点で限界に近い**（解像度 1280 幅などでは特に）。
- **第 7 画面** を同じやり方で足すと、**横幅・文字の見やすさ**がさらに厳しくなる見込みが高い。
...
- **第 6 画面（施設サマリー）** は **読み取り専用プロトタイプ**として追加済み。**第 7 画面** を足す前に、少なくとも **ホーム上部ナビの整理**（文言短さ・行の分け方など）を検討する余地がある。
...
- **既存 6 画面を一度に壊さない** ように、**少しずつ** 導線を整理する。
...
3. **その次** は **「第 7 画面に何を置くかの調査・判断」**（チームで優先度を決める）。カード型メニューは **小プロトタイプまで実施済み**。
4. **左サイドメニュー**は、**いま全面実装しない**。試すなら **1 画面だけ** や、**別ブランチ** など、**本線の 6 画面確認を壊しにくい**やり方にする（タイミングは **未確認**）。
...
- **第 7 画面の追加**（方針・ナビを固めないままの乱立）
- **既存 6 画面**の **大規模なレイアウト変更**

...
- **ホーム**: **`HeaderCard` のみ**に Theme を割当（**ルート `HomeDashboard` には付けない**）。**`Scroll` 以下**は **`CardNews`（`ed106c8`）と `CardNext`（`8676095`）**に `phase4_readonly_core.tres` + **`Phase4SummaryCard`** を付与済み。**それ以外**（`NavColumns`・主要指標・Tasks 等）は **従来の StyleBox のまま**。**HeaderNavRow** は **ボタン数・接続・遷移先のシーン定義を変更していない**（親 `HeaderCard` に Theme が付くため、**見た目**は Theme 継承で変わりうる）。

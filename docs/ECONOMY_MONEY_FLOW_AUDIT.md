...
| シーズン中（非正本） | `season.py` / `team.py` / `main_menu_view.py` | `Season._apply_inseason_league_distribution_round` → `record_inseason_league_distribution_round`；続けて `_apply_inseason_matchday_estimate_round` → `record_inseason_matchday_estimate_round` | 全 `all_teams` の `money` ＋各 `Team.inseason_cash_round_log` | 第 1: `INSEASON_LEAGUE_DISTRIBUTION_ROUND_YEN_BY_LEVEL`（`league_level` 別）。第 2: ホーム数×`INSEASON_MATCHDAY_ESTIMATE_ROUND_YEN_PER_HOME_GAME`（0 円ならログ行なし） | **分配按分**と**主場・門前概算**（CLI・GUI 一覧、`INSEASON_REVENUE_KEY_POLICY.md` §3） | **`finance_history` には載らない** | `simulate_next_round` 内・杯/EASL の後、CPU 裏経営の**前**。 |

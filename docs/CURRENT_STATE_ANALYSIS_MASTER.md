...
| 区分 | 内容 |
|------|------|
| 実装済み | スポンサー・広報・グッズ・施設・CPU 裏経営等のモジュールとテストが存在（git 履歴・ファイル存在）。**国内リーグ所属クラブ**のオフ国際大会賞金（杯・洲际・FINAL BOSS）は、オフ締め `record_financial_result` の `revenue` 内訳に合流（2026-04-06）。 |

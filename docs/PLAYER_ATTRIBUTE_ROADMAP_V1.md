...
## Phase 8 (Current Scope)

- 練習メニューに解放条件を導入（表示＋選択制限）
  - 個人ドリル: HCスタイル/施設Lv で一部ドリル解放
  - チーム練習: 特別メニュー（`precision_offense`, `intense_defense`）を追加
- 反映は引き続き薄く
  - 特別メニューのボーナスは既存より小幅上乗せ
  - いきなり大きな係数変更は行わない

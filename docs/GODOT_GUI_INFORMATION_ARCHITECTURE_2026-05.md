...
- **最新到達点（2026-05）**:
  ...
  - **今後の注意（本線移植）**: sandbox 固定文言（**`PO圏まで 2.0差`**、**`ホーム快勝、次戦へ弾み`** 等）を本線へ出すには **DTO / export / `.gd` の追加整理**が必要。**現時点では本線 Header の見た目**と **Scroll 以下の `CardNews` / `CardNext` の見た目**までを小さく移植した段階（**`news` の 1 行ヘッドライン本格化は未着手**）。**`CardWarnings` / `CardTasks` を `Phase4WarningCard` 化する前に、警告と ToDo の役割分界を確認する**。**`CardRank` / `CardMoney`（MetricsRow）は片方だけでなくセット方針で判断する**。本線移植判断では **from_python/mock**・**DataSourceLabel**・**HeaderNavRow**・**既存 10 画面導線を壊さないこと**を優先する。

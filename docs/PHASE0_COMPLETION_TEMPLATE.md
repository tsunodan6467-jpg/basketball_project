...
|---|------|------|------|---------|----------------|
| 1 | **ライセンス強制実機テスト** | **手順書作成済み**（2026-05-08、`docs/STEAM_LICENSE_REAL_DEVICE_TEST_PROCEDURE_2026-05.md`）／**実機実施待ち（人間作業）** | 実機作業＋（必要なら）docs | §3 ランタイム `[ ]`「強制終了ポリシーの実機テスト」 | 別 doc の手順書に従って Case A（購入済み・exit 0 系）／Case B（未購入・exit 3）／Case C（Steam 未起動・exit 2）を実機で実施 → §7 判定表を埋め、`reports/license_real_device_*.txt` の要点を本書に転記 → 完了条件（§8）を満たしたら `[x]` 化。本 PR は **手順書作成までで `[x]` 化はしない**。 |
...
| 3 | **ストア説明文への「セーブはローカル」明記** | **docs ドラフト作成済み**（2026-05-08、§4.7 にドラフト本文）／**パートナー画面反映待ち（人間作業）** | 人間作業（パートナー画面）＋ docs ドラフト | §3 セーブ（README 側は `[x]` 済）／ §5「ストア説明文（日本語）」`[ ]` | §4.7 のドラフト（短い注意書き版・1 行版・丁寧な説明版）から 1 つを選び、人間がパートナー画面のストア説明文へ貼り付け → 反映後に §5「ストア説明文（日本語）」を `[x]` に変える。本書は **docs ドラフト作成までで、`[x]` 化は実反映後**。 |

...
| **表示との整合** | CLI 1 行を **正式ラベル**に差し替え、可能なら**内訳の短い内訳**（分配 / 主場分）を括弧で示すと不透明感が下がる。**GUI**: 経営ウィンドウ財務サマリーで `inseason_cash_round_log` を一覧（`finance_report_display.format_inseason_cash_round_log_lines`）。 |

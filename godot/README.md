...
- **ホーム ↔ ロスター閲覧**: ホーム上部の **「ロスター閲覧へ（別画面・読取のみ）」** で `roster_view.tscn` へ。ロスター上部の **「ホームへ戻る（表示のみ）」** で `home_dashboard.tscn` へ。いずれも **画面切替のみ**（状態変更・Python 起動なし）。
- **ホーム ↔ クラブ史閲覧**: ホーム上部の **「クラブ史閲覧へ（別画面・読取のみ）」** で `club_history_view.tscn` へ。クラブ史上部の **「ホームへ戻る（表示のみ）」** で `home_dashboard.tscn` へ。**画面切替のみ**（進行・編集・保存はしない）。

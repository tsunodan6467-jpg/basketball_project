...
## 契約・人事サマリー閲覧（第10画面・読み取り専用）。
## Python 手動配置 JSON を優先し、無ければ同梱モックを読み込む。

var _contract_personnel_summary_json_paths: Array[String] = [
	"res://data/contract_personnel_summary_from_python.json",
	"res://data/contract_personnel_summary_mock.json",
]
...
func _load_snapshot() -> Dictionary:
	_last_loaded_uri = ""
	for path in _contract_personnel_summary_json_paths:
		if not FileAccess.file_exists(path):
			continue
		var f: FileAccess = FileAccess.open(path, FileAccess.READ)
		if f == null:
			push_warning("[contract_personnel_summary_view] Cannot open JSON, trying next: %s" % path)
			continue
		var text: String = f.get_as_text()
		var parsed: Variant = JSON.parse_string(text)
		if parsed == null:
			push_warning("[contract_personnel_summary_view] JSON parse failed, trying next: %s" % path)
			continue
		if typeof(parsed) != TYPE_DICTIONARY:
			push_warning("[contract_personnel_summary_view] JSON root is not an object, trying next: %s" % path)
			continue
		var data: Dictionary = parsed as Dictionary
		_last_loaded_uri = path
		print("[contract_personnel_summary_view] Loaded JSON from: ", path)
		return data
...
func _data_source_caption(uri: String) -> String:
	if uri.is_empty():
		return ""
	if uri.ends_with("contract_personnel_summary_from_python.json"):

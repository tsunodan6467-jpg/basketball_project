...
## オーナーミッション閲覧用（第8画面プロトタイプ）。同梱 mock JSON のみ読込。ホーム遷移は未接続。

const _MOCK_JSON_PATH := "res://data/owner_mission_mock.json"
...
func _load_mock_snapshot() -> Dictionary:
	...
	if f == null:
		push_warning("[owner_mission_view] Cannot open JSON: %s" % _MOCK_JSON_PATH)
		return {"_error": _LOAD_FAILED_MESSAGE}
	var text: String = f.get_as_text()
	var parsed: Variant = JSON.parse_string(text)
	if parsed == null:
		push_warning("[owner_mission_view] JSON parse failed: %s" % _MOCK_JSON_PATH)
		return {"_error": _LOAD_FAILED_MESSAGE}
	if typeof(parsed) != TYPE_DICTIONARY:
		return {"_error": _LOAD_FAILED_MESSAGE}
	print("[owner_mission_view] Loaded JSON from: ", _MOCK_JSON_PATH)

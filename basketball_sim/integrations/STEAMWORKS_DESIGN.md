...
### Phase 0 末期の決定チェックリスト（要否のすり合わせ用）

Phase 0 を一段落する前に、次を **はい／いいえ／後回し** で埋めて本書・`.cursorrules` に反映する。

1. **クラウドセーブ（Remote Storage またはハイブリッド）を v1（初回 Steam 正式リリース）に含めるか。** 含めない場合は、ストア／README で「セーブはローカル」と明記する方針を維持する。
2. **Rich Presence を v1 に含めるか。** 含める場合は **表示文言・ローカライズ・更新タイミング（試合中／メニュー）** の最小スコープを決める。含めない場合は v2 以降の候補に残す。
3. **初回リリースで Steam 側に必須とする機能の一覧**（例: ライセンスチェック・実績・`RunCallbacks` 安定・ローカルセーブ）を固定し、**「これが揃ったら Phase 0 の Steam 技術項は完了」**と定義する。
...
### Steam クライアントの起動と実行ファイル名（整合）

Steam クライアントから起動するときの **実行ファイル名**は、ビルド成果物と**一字一句揃える**（拡張子含む）。

| 項目 | このリポジトリの既定 |
|------|----------------------|
| PyInstaller 出力 | `BasketballGM.spec` の `EXE(..., name="BasketballGM", ...)` → `dist\BasketballGM.exe` |
| Inno ショートカット等 | `installer\BasketballGM.iss` の `{#MyAppExeName}`（既定 `BasketballGM.exe`） |
| パートナー画面の起動設定 | 上記と同じ **`BasketballGM.exe`** を指定する |

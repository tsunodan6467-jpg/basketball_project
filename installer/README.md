...
※ **Draft のまま**にしておけば、公開前に自分で Assets から `BasketballGM_Setup_*.exe` をダウンロードしてインストール確認してから **Publish** してもよい（ローカルで既に試したならそのまま公開で問題ありません）。

※補足: `Releases` の「閲覧ページ」にある `Assets` 欄は**追加できません**。ファイル添付は **`Edit release`（編集画面）** で行います。
...
- [ ] （インストーラを添付した場合）`BasketballGM_Setup_<version>.exe` を実行 → インストール完了 → スタートメニューまたはインストール先から起動できること

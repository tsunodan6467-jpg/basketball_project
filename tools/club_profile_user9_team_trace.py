"""
USER9（個別値9クラブ）を team_id 固定で年次追跡（観測専用・本体非改変）。

`club_profile_pilot_team_trace.py` と同型だが追跡対象のみ差し替え。

例:
  python tools/club_profile_user9_team_trace.py --years 3 --seed 424242 \\
    --out reports/club_profile_user9_team_trace_y3_s424242.txt
"""
...
TRACE_IDS: Tuple[int, ...] = (1, 2, 3, 4, 5, 7, 12, 13, 29)
...
def run_trace(*, years: int, seed: int, lines: List[str]) -> None:
    lines.append(f"club_profile_user9_team_trace | seed={seed} years={years}")
    ...
    lines.append("Notes: same semantics as club_profile_pilot_team_trace.py")


def main() -> int:
    ap = argparse.ArgumentParser(description="USER9 club_profile trace (9 clubs, by team_id)")
    ap.add_argument("--years", type=int, default=3)
    ap.add_argument("--seed", type=int, default=42_424_242)
    ap.add_argument(
        "--out",
        type=Path,
        default=Path("reports") / "club_profile_user9_team_trace.txt",
    )
    args = ap.parse_args()

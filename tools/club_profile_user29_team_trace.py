"""
USER29（個別値29クラブ）を team_id 固定で年次追跡（観測専用・本体非改変）。

例:
  python tools/club_profile_user29_team_trace.py --years 3 --seed 424242 \\
    --out reports/club_profile_user29_team_trace_y3_s424242.txt
"""
...
def _roster_mean_age_ovr(team: Any) -> Tuple[float | None, float | None]:
...
def _last_hist_rank_str(team: Any) -> str:
    hist = getattr(team, "history_seasons", None) or []
    if not hist:
        return "-"
    last = hist[-1]
...
def collect_trace_snapshots(*, years: int, seed: int) -> List[Dict[str, Any]]:
    ...
    def snap(label: str) -> None:
        ...
        for tid in TRACE_IDS:
            ...
            rw = max(0, int(getattr(team, "regular_wins", 0) or 0))
            rl = max(0, int(getattr(team, "regular_losses", 0) or 0))
            rows[tid] = {
                ...
                "age_mean": m["age_mean"],
                "ovr_mean": m["ovr_mean"],
                "rank_last": _last_hist_rank_str(team),
                "rw": rw,
                "rl": rl,
            }
            prev[tid] = {
...
def run_trace(*, years: int, seed: int, lines: List[str]) -> None:
    lines.append(f"club_profile_user29_team_trace | seed={seed} years={years}")
    lines.append(f"trace_team_ids={list(TRACE_IDS)}")
    lines.append("")
    ...
    for block in collect_trace_snapshots(years=years, seed=seed):
        ...
        hdr = (
            ...
            "cum_tin cum_tout cum_fa age_mean ovr_mean rank_last rw rl"
        )
        lines.append(hdr)
        for tid in TRACE_IDS:
            r = rows[tid]
            ...
            lines.append(
                ...
                f"{r['rank_last']} {r['rw']} {r['rl']}"
            )
        lines.append("")

    lines.append(
        "Notes: rank_last = last history_seasons[].rank if present; rw/rl = current regular record."
    )


def main() -> int:
    ap = argparse.ArgumentParser(description="USER29 club_profile trace (29 clubs, by team_id)")
    ap.add_argument("--years", type=int, default=3)
    ap.add_argument("--seed", type=int, default=42_424_242)
    ap.add_argument(
        "--out",
        type=Path,
        default=Path("reports") / "club_profile_user29_team_trace.txt",
    )
    args = ap.parse_args()

...
def _pre_le_population_summary_lines(rows: List[Dict[str, Any]]) -> List[str]:
    ...
    for r in rows:
        ...
        sc = d.get("soft_cap")
        pbg = d.get("payroll_before")
        cb = d.get("cap_base")
        if pap is not None and sc is not None and pbg is not None and cb is not None:
            pi, si = int(pap), int(sc)
            pbi = int(pbg)
            gate_payrolls.append(pi)
            sc_gate_vals.append(si)
            rts_gate_vals.append(max(0, si - pbi))
            cap_gate_vals.append(int(cb))
            if pi > si:
                n_pap_gt_sc += 1
            else:
                n_pap_le_sc += 1
    ...
    nabb = len(abb_vals)
    if nabb == 0:
        abb_line = "  offer_after_base_bonus n_abb=0"
    else:
        p25_a, p50_a, p75_a = _quartiles_int(abb_vals)
        abb_line = (
            "  offer_after_base_bonus "
            f"min={min(abb_vals)} max={max(abb_vals)} "
            f"p25={p25_a} p50={p50_a} p75={p75_a}"
        )
        if nabb != n:
    ...
    nbase = len(base_vals)
    if nbase == 0:
        base_line = "  base n_base=0"
    else:
        p25_bs, p50_bs, p75_bs = _quartiles_int(base_vals)
        base_line = (
            "  base "
            f"min={min(base_vals)} max={max(base_vals)} "
            f"p25={p25_bs} p50={p50_bs} p75={p75_bs}"
        )
        if nbase != n:
    ...
    if ng == 0:
        ...
        room_to_soft_gate_line = "  room_to_soft n_gate=0"
        cap_base_gate_line = "  cap_base n_gate=0"
    else:
        ...
        rts_u = set(rts_gate_vals)
        if len(rts_u) == 1:
            room_to_soft_gate_line = f"  room_to_soft value={rts_gate_vals[0]} (n_gate={ng})"
        else:
            p25_rts, p50_rts, p75_rts = _quartiles_int(rts_gate_vals)
            room_to_soft_gate_line = (
                "  room_to_soft "
                f"min={min(rts_gate_vals)} max={max(rts_gate_vals)} "
                f"p25={p25_rts} p50={p50_rts} p75={p75_rts} (n_gate={ng})"
            )
        cap_u = set(cap_gate_vals)
        if len(cap_u) == 1:
            cap_base_gate_line = f"  cap_base value={cap_gate_vals[0]} (n_gate={ng})"
        else:
            cap_base_gate_line = (
                "  cap_base "
                f"min={min(cap_gate_vals)} max={max(cap_gate_vals)} "
                f"unique={len(cap_u)} (n_gate={ng})"
            )
    npb = len(pb_vals)
    if npb == 0:

"""
USER21（個別値21クラブ）を team_id 固定で年次追跡（観測専用・本体非改変）。

例:
  python tools/club_profile_user21_team_trace.py --years 3 --seed 424242 \\
    --out reports/club_profile_user21_team_trace_y3_s424242.txt
"""
...
def run_trace(*, years: int, seed: int, lines: List[str]) -> None:
    lines.append(f"club_profile_user21_team_trace | seed={seed} years={years}")
    ...
    lines.append("Notes: same semantics as club_profile_user15_team_trace.py")


def main() -> int:
    ap = argparse.ArgumentParser(description="USER21 club_profile trace (21 clubs, by team_id)")
    ap.add_argument("--years", type=int, default=3)
    ap.add_argument("--seed", type=int, default=42_424_242)
    ap.add_argument(
        "--out",
        type=Path,
        default=Path("reports") / "club_profile_user21_team_trace.txt",
    )
    args = ap.parse_args()

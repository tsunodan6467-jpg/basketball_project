"""
パイロット 12 club の get_club_base_profile / payroll fac / 戦略曖昧帯の軽観測。

本体は変更しない。`offseason.compute_postoff_payroll_budget_with_temp_floor` の fac 式と
`cpu_club_strategy` の曖昧帯条件を読み取り専用で再掲する（本番と同じ式の複製）。
"""

from __future__ import annotations

import argparse
import sys
from pathlib import Path
from types import SimpleNamespace
from typing import Any, List, Tuple

ROOT = Path(__file__).resolve().parents[1]
if str(ROOT) not in sys.path:
    sys.path.insert(0, str(ROOT))

from basketball_sim.models.offseason import compute_postoff_payroll_budget_with_temp_floor  # noqa: E402
from basketball_sim.systems.club_profile import get_club_base_profile  # noqa: E402
from basketball_sim.systems.cpu_club_strategy import (  # noqa: E402
    _VALID_EXPECTATIONS,
    _resolve_strategy_tag,
    _tendency_scores,
    get_cpu_club_strategy,
)
from basketball_sim.systems.generator import generate_teams, sync_player_id_counter_from_world  # noqa: E402

PILOT_IDS = (1, 4, 8, 12, 13, 18, 21, 25, 29, 33, 38, 46)


def _payroll_fac(team: Any) -> float:
    """offseason.compute_postoff 内の fac と同一。"""
    try:
        if bool(getattr(team, "is_user_team", False)):
            return 1.0
        prof = get_club_base_profile(team)
        fac = (
            1.0
            + 0.022 * (float(prof.financial_power) - 1.0)
            + 0.018 * (float(prof.market_size) - 1.0)
            + 0.015 * (float(prof.arena_grade) - 1.0)
        )
        return max(0.97, min(1.03, float(fac)))
    except Exception:
        return 1.0


def _strategy_inputs(team: Any) -> Tuple[str, int, int, int]:
    exp = str(getattr(team, "owner_expectation", "playoff_race") or "playoff_race").strip().lower()
    if exp not in _VALID_EXPECTATIONS:
        exp = "playoff_race"
    ll = max(1, min(3, int(getattr(team, "league_level", 2) or 2)))
    wins = max(0, min(40, int(getattr(team, "regular_wins", 0) or 0)))
    money = max(0, int(getattr(team, "money", 0) or 0))
    return exp, ll, wins, money


def _nudge_preview(team: Any) -> Tuple[int, int, bool, str, str]:
    """曖昧帯か、net、プロファイルによる nudge 方向（push+/rebuild+/none）。"""
    exp, ll, wins, money = _strategy_inputs(team)
    ps, rs = _tendency_scores(team, exp, ll, wins, money)
    amb = -1 <= ps - rs <= 1
    prof = get_club_base_profile(team)
    net = float(prof.win_now_pressure) - float(prof.financial_power)
    if not amb:
        return ps, rs, False, f"net={net:.4f}", "n/a (not ambiguous)"
    if net > 0.035:
        return ps, rs, True, f"net={net:.4f}", "would +1 push_s"
    if net < -0.035:
        return ps, rs, True, f"net={net:.4f}", "would +1 reb_s"
    return ps, rs, True, f"net={net:.4f}", "no nudge (|net|<=0.035)"


def run_probe() -> List[str]:
    lines: List[str] = []
    teams = generate_teams()
    sync_player_id_counter_from_world(teams, [])

    lines.append("club_profile_pilot_probe")
    lines.append("")

    lines.append("=== 1) get_club_base_profile (pilot 12) ===")
    for tid in PILOT_IDS:
        t = next(x for x in teams if int(getattr(x, "team_id", 0) or 0) == tid)
        p = get_club_base_profile(t)
        lines.append(
            f"  team_id={tid} name={getattr(t, 'name', '')!r} -> "
            f"fin={p.financial_power} mkt={p.market_size} arena={p.arena_grade} "
            f"youth={p.youth_development_bias} win_now={p.win_now_pressure}"
        )

    lines.append("")
    lines.append("=== 1b) non-pilot sample (team_id 3, 5) ===")
    for tid in (3, 5):
        t = next(x for x in teams if int(getattr(x, "team_id", 0) or 0) == tid)
        p = get_club_base_profile(t)
        lines.append(
            f"  team_id={tid} name={getattr(t, 'name', '')!r} -> "
            f"fin={p.financial_power} mkt={p.market_size} arena={p.arena_grade} "
            f"youth={p.youth_development_bias} win_now={p.win_now_pressure}"
        )

    lines.append("")
    lines.append("=== 2) payroll fac (CPU) vs user stub ===")
    user_like = SimpleNamespace(
        team_id=99,
        league_level=3,
        is_user_team=True,
        market_size=1.0,
        popularity=50,
        sponsor_power=50,
        fan_base=5000,
        money=50_000_000,
    )
    lines.append(f"  user_like is_user_team=True fac={_payroll_fac(user_like):.6f} (expect 1.0)")
    for tid in PILOT_IDS:
        t = next(x for x in teams if int(getattr(x, "team_id", 0) or 0) == tid)
        lines.append(f"  pilot id={tid} fac={_payroll_fac(t):.6f}")

    lines.append("")
    lines.append("=== 2b) full budget roster_payroll=0 (pilot 12: id fac budget) ===")
    for tid in PILOT_IDS:
        t = next(x for x in teams if int(getattr(x, "team_id", 0) or 0) == tid)
        fac = _payroll_fac(t)
        b = compute_postoff_payroll_budget_with_temp_floor(t, 0)
        lines.append(f"  id={tid} fac={fac:.6f} budget={b}")

    lines.append("")
    lines.append("=== 2c) non-pilot fac/budget sample (roster_payroll=0) id=3 vs id=7 ===")
    t3 = next(x for x in teams if int(getattr(x, "team_id", 0) or 0) == 3)
    t7 = next(x for x in teams if int(getattr(x, "team_id", 0) or 0) == 7)
    lines.append(
        f"  id=3 fac={_payroll_fac(t3):.6f} budget={compute_postoff_payroll_budget_with_temp_floor(t3, 0)} "
        f"(推定プロファイル)"
    )
    lines.append(
        f"  id=7 fac={_payroll_fac(t7):.6f} budget={compute_postoff_payroll_budget_with_temp_floor(t7, 0)} "
        f"(推定プロファイル)"
    )

    lines.append("")
    lines.append("=== 3) strategy: ambiguous band + profile nudge preview ===")
    for tid in PILOT_IDS:
        t = next(x for x in teams if int(getattr(x, "team_id", 0) or 0) == tid)
        exp, ll, wins, money = _strategy_inputs(t)
        ps, rs = _tendency_scores(t, exp, ll, wins, money)
        tag = _resolve_strategy_tag(t, exp, ll, wins, money)
        _, _, amb, net_s, nudge = _nudge_preview(t)
        lines.append(
            f"  id={tid} push_s={ps} reb_s={rs} diff={ps - rs} ambiguous={amb} {net_s} -> {nudge} | tag={tag}"
        )

    amb_count = 0
    nudge_eligible = 0
    for t in teams:
        if getattr(t, "is_user_team", False):
            continue
        ps, rs, amb, _, nudge = _nudge_preview(t)
        if amb:
            amb_count += 1
            if "would +1" in nudge:
                nudge_eligible += 1
    lines.append("")
    lines.append(
        f"  scan 48 CPU: ambiguous_band_count={amb_count} "
        f"with_profile_nudge_direction_count={nudge_eligible}"
    )

    lines.append("")
    lines.append("=== 4) get_cpu_club_strategy (pilot 12) ===")
    for tid in PILOT_IDS:
        t = next(x for x in teams if int(getattr(x, "team_id", 0) or 0) == tid)
        s = get_cpu_club_strategy(t)
        lines.append(f"  id={tid} tag={s.strategy_tag} fa={s.fa_aggressiveness} trade_tol={s.trade_loss_tolerance}")

    lines.append("")
    lines.append("Done.")
    return lines


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--out", type=Path, default=Path("reports") / "club_profile_pilot_probe.txt")
    args = ap.parse_args()
    lines = run_probe()
    text = "\n".join(lines) + "\n"
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(text, encoding="utf-8")
    print(text)
    print(f"Wrote {args.out.resolve()}")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())

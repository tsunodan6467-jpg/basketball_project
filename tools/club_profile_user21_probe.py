"""
第1優先9 + 第2優先6 + 第3優先6 = 21クラブ個別 club_profile の薄い観測（本体非改変）。
...
# 個別値 override 21クラブ（第1+第2+第3優先）
USER21_IDS: Tuple[int, ...] = (
    1,
    2,
    3,
    4,
...
COMPARE_IDS: Tuple[int, ...] = (6, 9, 10, 20, 48)
...
def run_probe() -> List[str]:
    ...
    lines.append("club_profile_user21_probe (USER21 individual overrides + compare)")
    lines.append("")

    lines.append("=== 1) get_club_base_profile (USER21) ===")
    for tid in USER21_IDS:
    ...
    lines.append("")
    lines.append("=== 1b) compare: type-template clubs (not in USER21) ===")
    ...
    lines.append("")
    lines.append("=== 4) fac spread (USER21 only, sorted by fac desc) ===")
    rows = []
    for tid in USER21_IDS:
        t = next(x for x in teams if int(getattr(x, "team_id", 0) or 0) == tid)
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path, default=Path("reports") / "club_profile_user21_probe.txt")
    args = ap.parse_args()
    lines = run_probe()
    text = "\n".join(lines) + "\n"

"""
9クラブ個別 club_profile の薄い観測（本体非改変）。

get_club_base_profile / payroll fac（⑦式）/ 戦略曖昧帯＋プロファイル nudge / get_cpu_club_strategy。
比較用に team_id 6,8,14,15,48 を併記。
"""
...
USER9_IDS = (1, 2, 3, 4, 5, 7, 12, 13, 29)
COMPARE_IDS = (6, 8, 14, 15, 48)
...
def run_probe() -> List[str]:
    ...
    lines.append("club_profile_user9_probe (USER9 + compare sample)")
    lines.append("")

    lines.append("=== 1) get_club_base_profile (USER9) ===")
    for tid in USER9_IDS:
    ...
    lines.append("")
    lines.append("=== 1b) compare: type-template / pilot-only (not in USER9) ===")
    ...
    lines.append("")
    lines.append(
        "=== 3) strategy: tendency + opening_profile_hint(push_h,reb_h) + ambiguous nudge preview + tag + scalars ==="
    )
    for tid in USER9_IDS + (6, 14):
        t = next(x for x in teams if int(getattr(x, "team_id", 0) or 0) == tid)
    ...
    lines.append("")
    lines.append("=== 4) fac spread (USER9 only, sorted by fac desc) ===")
    rows = []
    for tid in USER9_IDS:
        t = next(x for x in teams if int(getattr(x, "team_id", 0) or 0) == tid)
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path, default=Path("reports") / "club_profile_user9_probe.txt")
    args = ap.parse_args()
    lines = run_probe()
    text = "\n".join(lines) + "\n"

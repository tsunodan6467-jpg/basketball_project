...
# 個別値 override 29クラブ（第1+第2+第3+第4優先）
USER29_IDS: Tuple[int, ...] = (
    1,
    2,
    3,
    4,
...
# 型テンプレのみ（29 に含めない id）。9/10 は第4優先で個別化済みのため避ける。
COMPARE_IDS: Tuple[int, ...] = (23, 24, 32, 48)
...
def run_probe() -> List[str]:
    ...
    lines.append("club_profile_user29_probe (USER29 individual overrides + compare)")
    lines.append("")

    lines.append("=== 1) get_club_base_profile (USER29) ===")
    for tid in USER29_IDS:
    ...
    lines.append("")
    lines.append("=== 1b) compare: type-template only (not in USER29) ===")
    ...
    for tid in USER29_IDS + (23, 24, 32):
        t = next(x for x in teams if int(getattr(x, "team_id", 0) or 0) == tid)
    ...
    lines.append("")
    lines.append("=== 4) fac spread (USER29 only, sorted by fac desc) ===")
    rows = []
    for tid in USER29_IDS:
        t = next(x for x in teams if int(getattr(x, "team_id", 0) or 0) == tid)
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path, default=Path("reports") / "club_profile_user29_probe.txt")
    args = ap.parse_args()
    lines = run_probe()
    text = "\n".join(lines) + "\n"
